	...
	public function OnAfterSetUserGroup($USER_ID, $arGroups)
	{
		if (is_array(current($arGroups))) {
			...
			foreach($arGroups as $key => $arGroup) {
				if ($arGroup['DATE_ACTIVE_FROM'] != '' && $date < intval(\Bitrix\Main\Type\Date::createFromText($arGroup['DATE_ACTIVE_FROM'])->format('Ymd'))){
					continue;
				}
				if ($arGroup['DATE_ACTIVE_TO'] != '' && $date > intval(\Bitrix\Main\Type\Date::createFromText($arGroup['DATE_ACTIVE_TO'])->format('Ymd'))){
					continue;
				}
				$arGroupsPre[] = $key;
			}
